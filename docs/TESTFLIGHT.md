...
| 1.0.3 (28) | 2026-05-20 | Sentry signal-to-noise: tag http.url/method/code em axios errors, demote transient timeouts para warning. Supply-chain hardening: axios ^1.13.6 -> ^1.15.1 (npm resolve 1.16.1, fora do CVE-2025-62718 e da janela de compromise mar/26). Pin exato tanstack react-query 5.100.7. npm audit fix mobile (high 4 -> 0) + backend (critical 1 -> 0, high 4 -> 1 com drizzle deferido). **ITMS-90189**: 28 já usado em ASC, substituído por 29. |
...
| 1.0.3 (33) | 2026-05-21 | Banner ainda persistente em 32. Disabled NetworkBanner (return null) como kill-switch enquanto investigação. EmptyState em listas continua cobrindo offline UX granular. |
| 1.0.3 (34) | 2026-05-21 | **Root cause encontrado**: `@react-native-community/netinfo` 11.4.1 NÃO suporta New Architecture (newArchEnabled=true). Native bridge não conecta, JS sempre recebe `isConnected: false`. Fix: bump pra **12.0.1** (New Arch support desde 11.5.0, fix iOS 11.5.1). Restaurado NetworkBanner. `NetInfo.configure({reachabilityUrl: clients3.google.com/generate_204})` substitui probe Apple. Trata `isConnected === null` como unknown. |
| 1.0.3 (35) | 2026-05-21 | NetInfo agora funciona, mas banner ficava com tira amarela residual ao subir (translateY -80 insuficiente vs safe-area do iPhone com Dynamic Island). Fix: trocar animação para opacity fade in/out + `setMounted(false)` após fade-out → componente sai do tree por completo. |
| 1.0.4 (36) | 2026-06-10 | **Multi-família**: pessoa pode ser guardian em várias famílias (migração 0010 — unique por família). Aceite de convite com conta existente (entra como guardian), `/families/memberships` + `/families/switch`, seletor "Minhas famílias". Convite aceita Google/Apple. Onboarding gateia limite de filhos do plano. Fix segurança: bloqueia `/auth/register` com e-mail já usado como guardian (account takeover). Entitlement respeita `expiresAt` com graça de 3 dias. **Bump 1.0.3 → 1.0.4** (1.0.3 já aprovado na ASC). runtimeVersion string explícito p/ OTA em bare workflow. |
| 1.0.4 (37) | 2026-06-10 20:46 | Re-upload apos ITMS-90189 no build 36. Mesmo conteudo do 36; script corrigido para usar buildNumber persistente e executar apenas um export/upload por tentativa. |
